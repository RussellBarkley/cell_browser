...
We sampled large regions (~40mm² area) of no. 1.5 glass coverslips using a motorized stage on a laser scanning confocal microscope (Olympus Fluoview FV3000) with an oil immersion objective lens at 100X magnification (Olympus, model UPLAPO100X, NA 1.50). The pinhole diameter was opened to 800μm to maximize the depth of field. N=2500 fields were imaged in a 50x50 tile grid (snake pattern by rows: right down left down) with 10% overlap. Z-drift compensation was active to maintain focus and the confocal microscope was atop a pressurized CleanBench isolation laboratory table (TMC Vibration Control) to minimize environmental vibrations.
...
## 2) Tile stitching with Microscopy Image Stitching Tool (MIST)

The size of a full stitched image approaches the maximum value that a 32-bit integer can hold, therefore the N=2500 .tif files were divided into sub-folders containing n=625 images corresponding to four quarters using {ref}`sort-quarters`. The images in these sub-folders were stitched into 25x25 tile grids using the MIST plugin [@doi:10.1038/s41598-017-04567-y] in FIJI. MIST was useful to correct for inaccurate step sizes that sometimes occurred with the motorized stage.
...
The stitched quarter was displayed and saved, named by run and position. For example, Run53TL was from the top left quarter of the 53rd imaging experiment. The stitched quarters were then cropped into four quarters using {ref}`crop-quarters`, yielding n=16 stitched images per coverslip, and N=1600 stitched images from all runs.
...
```{admonition} We recognize that our data processing methods introduced bias
:class: warning
...
The orientation of a cell is known to confound the vector embedding of autoencoder models trained on single-cell microscopy data, motivating the development of orientation-invariant autoencoder models [@doi:10.1038/s41467-024-45362-4]. Similarly, a multi-encoder variational autoencoder model controlled for several transformational features like orientation that were 'uninformative' in single-cell analyses [@doi:10.1038/s42003-022-03218-x]. With {ref}`crop-rois`, we [pre-aligned](https://github.com/jmhb0/o2vae/tree/master/prealignment) and center-cropped the nucleus segmentation masks by fitting a minimal area rectangle to the mask, then all values outside of the mask were set to zero. The dimensions of the cropped images are 256x256 pixels, which is one-quarter the size of the uncompressed 1024x1024 pixel microscopy fields.